<p align="center">
  <img src="img/logo-supervibe.png" alt="SIA Ultimate - Max Supervibe" width="380">
</p>

## Connect your SIA-DCS compatible alarm system to node-red.

SIA Ultimate connects SIA-DC09 compatible alarm systems, including Ajax Systems, directly to Node-RED. Receive real-time alarm events, monitor security status, and integrate intrusion, fire, and technical alarms into powerful automation workflows, enabling seamless interaction with smart home and building management systems.
...
<p align="center">
  <img src="img/readmemain.png" alt="SIA Ultimate for Node-RED — Max Supervibe" width="70%">
</p>
...
- **Connection close by SIA Server**: if enabled, the node closes the TCP connection after sending the ACK (Default enabled). If disabled, the node waits for the alarm panel to close the TCP connection (useful for panels that require stricter TCP handling after specific events)
- **Close delay after ACK (ms)**: delay before closing the TCP connection after sending the ACK (only if _Connection close by SIA Server_ is enabled, Default 700ms)
- **Wait panel close timeout (sec)**: maximum time to wait for the alarm panel to close the TCP connection (only if _Connection close by SIA Server_ is disabled, Default 30s). After this timeout the node will close the socket
...
- **SIA-DC09 Device List**: You can import your own list of device names and device ids. The node will emit the device name based on device ID in the SIA message. Currently compatible only with **_SIA-DC09_**. For example:
  - 0000,Alarm Panel
  - 1,PIR Bedrom
  - 2,Microwave Front Door
  - 4,PIR Soggiorno
...
- **FILTERS**
  - **Discard test messages**: your alarm panel can send an automatic test message once a time, to check that the SIA-DCS node is connected. You can avoid the node sending a msg to the flow everytime this automatuc test message is received. The node will still emit an ERROR message on the PIN 2, if it doesn't receive any message during the period specified in _Emit error if no messages arrive within seconds_ parameter.
...
msg = {
  ...
  topic: "Banano",
  payload: {
    // This contains the message decoded
