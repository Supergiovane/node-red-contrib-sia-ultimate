...
* **Close delay after ACK (ms)**: delay before closing the TCP connection after sending the ACK (only if *Connection close by SIA Server* is enabled, Default 700ms)
* **Wait panel close timeout (sec)**: maximum time to wait for the alarm panel to close the TCP connection (only if *Connection close by SIA Server* is disabled, Default 30s). After this timeout the node will close the socket
